...
def welcomeScreen():
    """ Shows welcomes images on the screen """
    ...
    while True:
        for event in pygame.event.get():
            # if user click on cross the button close the game
            if event.type == QUIT or (event.type == KEYDOWN and event.key == K_ESCAPE):
                pygame.quit()
                sys.exit()

            # if user click space or up key, start the game
...
def mainGame():
    ...
    while True:
        ...
        for pipe in upperPipes:
            if pipe['x'] + GAME_SPRITES['pipe'].get_width()/2 < playerx + GAME_SPRITES['bird'].get_width()/2 < pipe['x'] + GAME_SPRITES['pipe'].get_width()/2 + 4 :
                score += 1
...
if __name__ == "__main__":
    # This will be the main point froom where our game will start
    pygame.init()  # Initialize all pygames modules
    FPSCLOCK = pygame.time.Clock()
    pygame.display.set_caption('Flappy Bird by Loki')
